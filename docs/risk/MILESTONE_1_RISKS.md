...
| `L-13` | Current Delhi government pages mix the 2001 regime, national Act workflow, and even the wrong Central portal | High | Critical | Quarantine legacy sources; hard-separate Central and Delhi routing; require legal/RTI review before activation | Delhi profile tests reject legacy fees and Central portal routing |
| `L-14` | Haryana requires identity proof, creating rejection and sensitive-document handling risk | High | Critical | Show the requirement; local-only processing; separate manual attachment path; forbid IDs in logs, analytics, URLs, fixtures and Git | Privacy review plus network/log/source-control leakage tests |
| `L-15` | Goa's 2008 Rule 4 can apply higher fees under another law, while its portal FAQ cites unexplained 2012 rules | Medium | High | Warn that later fees may vary; preserve fee-appeal path; block filing-ready status until the rule reference is reconciled | Legal/RTI-approved Goa rule-chain and portal conflict record |
